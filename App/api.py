...
def ajaxfile():
    ...
    try:
        if request.method == 'POST':
            ...
            ## Fetch records
            if searchValue=='':
                cursor.execute("SELECT * FROM products ORDER BY  productID   asc limit %s, %s ;", (row, rowperpage))
                employeelist = cursor.fetchall()
            else:        
                cursor.execute("SELECT * FROM products  WHERE name LIKE %s OR productID LIKE %s  ORDER BY productID   limit %s, %s ;", (likeString, likeString, row, rowperpage))
                employeelist = cursor.fetchall()
